...
namespace GT_CustomMapSupportRuntime
{
	...
	public class AIAgent : MapEntity
	{
		...
		{
			if (this.agentBehaviours.Count > 3)
			{
				this.agentBehaviours.RemoveRange(3, this.agentBehaviours.Count - 3);
			}
			try
			{
				AIAgent.validateBehaviors.Clear();
				AIAgent.invalidEntries.Clear();
				if (this.agentBehaviours.Count != 0)
				{
					for (int i = 0; i < this.agentBehaviours.Count; i++)
					{
						if (!AIAgent.validateBehaviors.Contains(this.agentBehaviours[i]))
						{
							AIAgent.validateBehaviors.Add(this.agentBehaviours[i]);
						}
						else
						{
							AIAgent.invalidEntries.Add(i);
						}
					}
					foreach (int index in AIAgent.invalidEntries)
					{
						for (int j = 0; j < 3; j++)
						{
							if (!AIAgent.validateBehaviors.Contains((AgentBehaviours)j))
							{
								this.agentBehaviours[index] = (AgentBehaviours)j;
								AIAgent.validateBehaviors.Add(this.agentBehaviours[index]);
								break;
							}
						}
					}
		...
		public override long GetPackedCreateData()
		{
			...
			return (long)((ulong)(hasInstance ? this.enemyTypeId : this.entityTypeId) + (ulong)((long)((long)(hasInstance ? this.lua_AgentID : this.lua_EntityID) << 8)));
		}
