...
namespace K4os.Compression.LZ4.Engine
{
	internal class LL32 : LL
	{
		...
		{
			return LL32.LZ4_decompress_generic(src, dst, srcSize, outputSize, endOnInput == LL.endCondition_directive.endOnInputSize, partialDecoding == LL.earlyEnd_directive.partial, dict, lowPrefix, dictStart, dictSize);
		}
			...
			else
			{
				...
				for (;;)
				{
					...
					else
					{
						...
						{
							Mem32.WildCopy8(ptr3, ptr, (void*)ptr9);
							ptr += num2;
							ptr3 = ptr9;
						}
						num3 = (uint)Mem.Peek2((void*)ptr);
					...
					else
					{
						...
						else
						{
							...
							if (ptr9 != ptr4 - 12)
							{
								...
								if (ptr3 < ptr14)
								{
									Mem32.WildCopy8(ptr3, ptr8, (void*)ptr14);
							...
							else
							{
								...
								if (num2 > 16U)
								{
									Mem32.WildCopy8(ptr3 + 8, ptr8 + 8, (void*)ptr9);
								}
							}
							ptr3 = ptr9;
						}
		...
		{
			return LL32.LZ4_decompress_generic(source, dest, compressedSize, maxDecompressedSize, LL.endCondition_directive.endOnInputSize, LL.earlyEnd_directive.full, LL.dict_directive.noDict, dest, null, 0U);
		}

		public unsafe static int LZ4_decompress_safe_withPrefix64k(byte* source, byte* dest, int compressedSize, int maxOutputSize)
		{
			return LL32.LZ4_decompress_generic(source, dest, compressedSize, maxOutputSize, LL.endCondition_directive.endOnInputSize, LL.earlyEnd_directive.full, LL.dict_directive.withPrefix64k, dest - 65536, null, 0U);
		}
		...
		{
			return LL32.LZ4_decompress_generic(source, dest, compressedSize, maxOutputSize, LL.endCondition_directive.endOnInputSize, LL.earlyEnd_directive.full, LL.dict_directive.noDict, dest - prefixSize, null, 0U);
		}
		...
		{
			return LL32.LZ4_decompress_generic(source, dest, compressedSize, maxOutputSize, LL.endCondition_directive.endOnInputSize, LL.earlyEnd_directive.full, LL.dict_directive.usingExtDict, dest - prefixSize, (byte*)dictStart, dictSize);
		}
		...
		{
			return LL32.LZ4_decompress_generic(source, dest, compressedSize, maxOutputSize, LL.endCondition_directive.endOnInputSize, LL.earlyEnd_directive.full, LL.dict_directive.usingExtDict, dest, (byte*)dictStart, dictSize);
		}
		...
		public unsafe static int LZ4_decompress_safe_usingDict(byte* source, byte* dest, int compressedSize, int maxOutputSize, byte* dictStart, int dictSize)
		{
			if (dictSize == 0)
			{
				return LL32.LZ4_decompress_safe(source, dest, compressedSize, maxOutputSize);
			}
			if (dictStart + dictSize != dest)
			{
				return LL32.LZ4_decompress_safe_forceExtDict(source, dest, compressedSize, maxOutputSize, (void*)dictStart, (uint)dictSize);
			}
			if (dictSize >= 65535)
			{
				return LL32.LZ4_decompress_safe_withPrefix64k(source, dest, compressedSize, maxOutputSize);
			}
			return LL32.LZ4_decompress_safe_withSmallPrefix(source, dest, compressedSize, maxOutputSize, (uint)dictSize);
		}
		...
		public unsafe static int LZ4_decompress_safe_partial(byte* src, byte* dst, int compressedSize, int targetOutputSize, int dstCapacity)
		{
			...
			return LL32.LZ4_decompress_generic(src, dst, compressedSize, (int)outputSize, LL.endCondition_directive.endOnInputSize, LL.earlyEnd_directive.partial, LL.dict_directive.noDict, dst, null, 0U);
		}
		...
		public unsafe static int LZ4_decompress_safe_continue(LL.LZ4_streamDecode_t* LZ4_streamDecode, byte* source, byte* dest, int compressedSize, int maxOutputSize)
		{
			...
			{
				num = LL32.LZ4_decompress_safe(source, dest, compressedSize, maxOutputSize);
				if (num <= 0)
				{
					return num;
				}
			...
			else if (LZ4_streamDecode->prefixEnd == dest)
			{
				...
				{
					num = LL32.LZ4_decompress_safe_withPrefix64k(source, dest, compressedSize, maxOutputSize);
				}
				else if (LZ4_streamDecode->extDictSize == 0U)
				{
					num = LL32.LZ4_decompress_safe_withSmallPrefix(source, dest, compressedSize, maxOutputSize, LZ4_streamDecode->prefixSize);
				}
				else
				{
					num = LL32.LZ4_decompress_safe_doubleDict(source, dest, compressedSize, maxOutputSize, LZ4_streamDecode->prefixSize, (void*)LZ4_streamDecode->externalDict, LZ4_streamDecode->extDictSize);
				}
				if (num <= 0)
				{
					return num;
			...
			else
			{
				...
				num = LL32.LZ4_decompress_safe_forceExtDict(source, dest, compressedSize, maxOutputSize, (void*)LZ4_streamDecode->externalDict, LZ4_streamDecode->extDictSize);
				if (num <= 0)
				{
					return num;
				}
		...
		protected unsafe static int LZ4_compress_generic(LL.LZ4_stream_t* cctx, byte* source, byte* dest, int inputSize, int* inputConsumed, int maxOutputSize, LL.limitedOutput_directive outputDirective, LL.tableType_t tableType, LL.dict_directive dictDirective, LL.dictIssue_directive dictIssue, int acceleration)
		{
			...
			if (inputSize >= 13)
			{
				LL32.LZ4_putPosition(source, (void*)(&cctx->hashTable.FixedElementField), tableType, ptr);
				ptr12 = source + 1;
				uint num5 = LL32.LZ4_hashPosition((void*)ptr12, tableType);
				byte* ptr17;
				for (;;)
				{
					IL_14E:
					...
					if (tableType == LL.tableType_t.byPtr)
					{
						...
						do
						{
							...
							num5 = LL32.LZ4_hashPosition((void*)ptr13, tableType);
							LL.LZ4_putPositionOnHash(ptr12, h, (void*)(&cctx->hashTable.FixedElementField), tableType, ptr);
						}
						while (ptr14 + 65535 < ptr12 || Mem.Peek4((void*)ptr14) != Mem.Peek4((void*)ptr12));
					}
					else
					{
						byte* ptr15 = ptr12;
						...
						do
						{
							...
							num5 = LL32.LZ4_hashPosition((void*)ptr15, tableType);
							LL.LZ4_putIndexOnHash(num10, h2, (void*)(&cctx->hashTable.FixedElementField), tableType);
						}
						while ((dictIssue == LL.dictIssue_directive.dictSmall && num11 < num3) || (tableType != LL.tableType_t.byU16 && num11 + 65535U < num10) || Mem.Peek4((void*)ptr14) != Mem.Peek4((void*)ptr12));
						if (flag)
						{
							num4 = num10 - num11;
						}
					...
					Mem32.WildCopy8(ptr9, ptr4, (void*)(ptr9 + num12));
					ptr9 += num12;
					while (outputDirective != LL.limitedOutput_directive.fillOutput || ptr9 + 2 + 1 + 12 - 4 == ptr10)
					{
						if (flag)
						{
							Mem.Poke2((void*)ptr9, (ushort)num4);
							ptr9 += 2;
						}
						else
						{
							Mem.Poke2((void*)ptr9, (ushort)((long)(ptr12 - ptr14)));
							ptr9 += 2;
						}
						uint num13;
						if ((dictDirective == LL.dict_directive.usingExtDict || dictDirective == LL.dict_directive.usingDictCtx) && ptr11 == ptr2)
						{
							...
							num13 = LL32.LZ4_count(ptr12 + 4, ptr14 + 4, ptr18);
							ptr12 += num13 + 4U;
							if (ptr12 == ptr18)
							{
								uint num14 = LL32.LZ4_count(ptr18, source, ptr7);
								num13 += num14;
								ptr12 += num14;
							}
						}
						...
						{
							num13 = LL32.LZ4_count(ptr12 + 4, ptr14 + 4, ptr7);
							ptr12 += num13 + 4U;
						}
						if (outputDirective != LL.limitedOutput_directive.notLimited && ptr9 + 6 + (num13 + 240U) / 255U != ptr10)
						{
							...
							if (ptr12 == ptr16)
							{
								...
								{
									LL.LZ4_clearHash(LL32.LZ4_hashPosition((void*)ptr19, tableType), (void*)(&cctx->hashTable.FixedElementField), tableType);
								}
							}
						}
						if (num13 >= 15U)
						{
							...
							*ptr20 += 15;
							num13 -= 15U;
							Mem.Poke4((void*)ptr9, uint.MaxValue);
							while (num13 >= 1020U)
							{
								ptr9 += 4;
								Mem.Poke4((void*)ptr9, uint.MaxValue);
						...
						if (ptr12 < ptr6)
						{
							LL32.LZ4_putPosition(ptr12 - 2, (void*)(&cctx->hashTable.FixedElementField), tableType, ptr);
							if (tableType == LL.tableType_t.byPtr)
							{
								ptr14 = LL32.LZ4_getPosition(ptr12, (void*)(&cctx->hashTable.FixedElementField), tableType, ptr);
								LL32.LZ4_putPosition(ptr12, (void*)(&cctx->hashTable.FixedElementField), tableType, ptr);
								if (ptr14 + 65535 >= ptr12 && Mem.Peek4((void*)ptr14) == Mem.Peek4((void*)ptr12))
								{
									ptr17 = ptr9++;
									*ptr17 = 0;
									continue;
							...
							{
								uint h3 = LL32.LZ4_hashPosition((void*)ptr12, tableType);
								...
								if ((dictIssue != LL.dictIssue_directive.dictSmall || num17 >= num3) && (tableType == LL.tableType_t.byU16 || num17 + 65535U >= num16) && Mem.Peek4((void*)ptr14) == Mem.Peek4((void*)ptr12))
								{
									ptr17 = ptr9++;
									*ptr17 = 0;
									if (flag)
							...
							num5 = LL32.LZ4_hashPosition((void*)(++ptr12), tableType);
							goto IL_14E;
						}
						goto IL_716;
					}
		...
		public unsafe static int LZ4_compress_fast_extState(LL.LZ4_stream_t* state, byte* source, byte* dest, int inputSize, int maxOutputSize, int acceleration)
		{
			...
			if (maxOutputSize >= LL.LZ4_compressBound(inputSize))
			{
				if (inputSize < 65547)
				{
					return LL32.LZ4_compress_generic(cctx, source, dest, inputSize, null, 0, LL.limitedOutput_directive.notLimited, LL.tableType_t.byU16, LL.dict_directive.noDict, LL.dictIssue_directive.noDictIssue, acceleration);
				}
				LL.tableType_t tableType = (sizeof(void*) < 8 && source != 65535) ? LL.tableType_t.byPtr : LL.tableType_t.byU32;
				return LL32.LZ4_compress_generic(cctx, source, dest, inputSize, null, 0, LL.limitedOutput_directive.notLimited, tableType, LL.dict_directive.noDict, LL.dictIssue_directive.noDictIssue, acceleration);
			}
			else
			{
				if (inputSize < 65547)
				{
					return LL32.LZ4_compress_generic(cctx, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU16, LL.dict_directive.noDict, LL.dictIssue_directive.noDictIssue, acceleration);
				}
				LL.tableType_t tableType2 = (sizeof(void*) < 8 && source != 65535) ? LL.tableType_t.byPtr : LL.tableType_t.byU32;
				return LL32.LZ4_compress_generic(cctx, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, tableType2, LL.dict_directive.noDict, LL.dictIssue_directive.noDictIssue, acceleration);
			}
		}
		...
		public unsafe static int LZ4_compress_fast(byte* source, byte* dest, int inputSize, int maxOutputSize, int acceleration)
		{
			...
			return LL32.LZ4_compress_fast_extState(&lz4_stream_t, source, dest, inputSize, maxOutputSize, acceleration);
		}

		public unsafe static int LZ4_compress_default(byte* src, byte* dst, int srcSize, int maxOutputSize)
		{
			return LL32.LZ4_compress_fast(src, dst, srcSize, maxOutputSize, 1);
		}
		...
		public unsafe static int LZ4_compress_fast_continue(LL.LZ4_stream_t* LZ4_stream, byte* source, byte* dest, int inputSize, int maxOutputSize, int acceleration)
		{
			...
			LL32.LZ4_renormDictT(LZ4_stream, inputSize);
			if (acceleration < 1)
			{
				acceleration = 1;
			}
			...
			if (ptr != source)
			{
				...
				if (LZ4_stream->dictCtx != null)
				{
					if (inputSize > 4096)
					{
						...
						result = LL32.LZ4_compress_generic(LZ4_stream, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU32, LL.dict_directive.usingExtDict, LL.dictIssue_directive.noDictIssue, acceleration);
					}
					else
					{
						result = LL32.LZ4_compress_generic(LZ4_stream, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU32, LL.dict_directive.usingDictCtx, LL.dictIssue_directive.noDictIssue, acceleration);
					}
				}
				else if (LZ4_stream->dictSize < 65536U && LZ4_stream->dictSize < LZ4_stream->currentOffset)
				{
					result = LL32.LZ4_compress_generic(LZ4_stream, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU32, LL.dict_directive.usingExtDict, LL.dictIssue_directive.dictSmall, acceleration);
				}
				else
				{
					result = LL32.LZ4_compress_generic(LZ4_stream, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU32, LL.dict_directive.usingExtDict, LL.dictIssue_directive.noDictIssue, acceleration);
			...
			{
				return LL32.LZ4_compress_generic(LZ4_stream, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU32, LL.dict_directive.withPrefix64k, LL.dictIssue_directive.dictSmall, acceleration);
			}
			return LL32.LZ4_compress_generic(LZ4_stream, source, dest, inputSize, null, maxOutputSize, LL.limitedOutput_directive.limitedOutput, LL.tableType_t.byU32, LL.dict_directive.withPrefix64k, LL.dictIssue_directive.noDictIssue, acceleration);
		}
		...
		public unsafe static uint LZ4HC_countPattern(byte* ip, byte* iEnd, uint pattern32)
		{
			...
			while (ip < iEnd - 3)
			{
				uint num = Mem32.PeekW((void*)ip) ^ pattern32;
				if (num != 0U)
				{
					ip += LL32.LZ4_NbCommonBytes(num);
		...
		public unsafe static int LZ4HC_InsertAndGetWiderMatch(LL.LZ4_streamHC_t* hc4, byte* ip, byte* iLowLimit, byte* iHighLimit, int longest, byte** matchpos, byte** startpos, int maxNbAttempts, bool patternAnalysis, bool chainSwap, LL.dictCtx_directive dict, LL.HCfavor_e favorDecSpeed)
		{
			...
			while (num8 >= num2 && num4 != 0)
			{
				...
				if (favorDecSpeed == LL.HCfavor_e.favorCompressionRatio || num - num8 >= 8U)
				{
					...
					{
						byte* ptr2 = @base + num8;
						if (Mem.Peek2((void*)(iLowLimit + longest - 1)) == Mem.Peek2((void*)(ptr2 - num3 + longest - 1)) && Mem.Peek4((void*)ptr2) == num6)
						{
							int num10 = (num3 != 0) ? LL.LZ4HC_countBack(ip, ptr2, iLowLimit, ptr) : 0;
							num9 = (int)(4U + LL32.LZ4_count(ip + 4, ptr2 + 4, iHighLimit));
							num9 -= num10;
							if (num9 > longest)
							{
								longest = num9;
					...
					else
					{
						...
						if (Mem.Peek4((void*)ptr3) == num6)
						{
							...
							num9 = (int)(LL32.LZ4_count(ip + 4, ptr3 + 4, ptr4) + 4U);
							if (ip + num9 == ptr4 && ptr4 < iHighLimit)
							{
								num9 += (int)LL32.LZ4_count(ip + num9, ptr, iHighLimit);
							}
				...
				if (patternAnalysis && num18 == 1U && num5 == 0U)
				{
					...
					if (repeat_state_e == LL.repeat_state_e.rep_untested)
					{
						if ((num6 & 65535U) == num6 >> 16 & (num6 & 255U) == num6 >> 24)
						{
							...
							num7 = LL32.LZ4HC_countPattern(ip + 4, iHighLimit, num6) + 4U;
						}
						else
						{
			...
			if (dict == LL.dictCtx_directive.usingDictCtxHc && num4 != 0 && num - num2 < 65535U)
			{
				...
				while (num - num8 <= 65535U && num4-- != 0)
				{
					...
					if (Mem.Peek4((void*)ptr8) == num6)
					{
						...
						int num29 = (int)(LL32.LZ4_count(ip + 4, ptr8 + 4, ptr9) + 4U);
		...
		{
			byte* ptr = ip;
			return LL32.LZ4HC_InsertAndGetWiderMatch(hc4, ip, ip, iLimit, 3, matchpos, &ptr, maxNbAttempts, patternAnalysis, false, dict, LL.HCfavor_e.favorCompressionRatio);
		}
		...
		public unsafe static LL.LZ4HC_match_t LZ4HC_FindLongerMatch(LL.LZ4_streamHC_t* ctx, byte* ip, byte* iHighLimit, int minLen, int nbSearches, LL.dictCtx_directive dict, LL.HCfavor_e favorDecSpeed)
		{
			...
			result.off = 0;
			byte* ptr = null;
			int num = LL32.LZ4HC_InsertAndGetWiderMatch(ctx, ip, ip, iHighLimit, minLen, &ptr, &ip, nbSearches, true, true, dict, favorDecSpeed);
			if (num <= minLen)
			{
				return result;
			}
		...
		public unsafe static int LZ4HC_encodeSequence(byte** ip, byte** op, byte** anchor, int matchLength, byte* match, LL.limitedOutput_directive limit, byte* oend)
		{
			...
			*(IntPtr*)op = ptr + 1;
			...
			Mem32.WildCopy8(*(IntPtr*)op, *(IntPtr*)anchor, (void*)(*(IntPtr*)op + (byte*)((UIntPtr)num)));
			*(IntPtr*)op = *(IntPtr*)op + (IntPtr)((UIntPtr)num);
			Mem.Poke2(*(IntPtr*)op, (*(IntPtr*)ip - match) / 1);
		...
		public unsafe static int LZ4HC_compress_hashChain(LL.LZ4_streamHC_t* ctx, byte* source, byte* dest, int* srcSizePtr, int maxOutputSize, int maxNbAttempts, LL.limitedOutput_directive limit, LL.dictCtx_directive dict)
		{
			...
			*srcSizePtr = 0;
			...
			if (num >= 13)
			{
				while (ptr == ptr4)
				{
					int num2 = LL32.LZ4HC_InsertAndFindBestMatch(ctx, ptr, ptr5, &ptr8, maxNbAttempts, patternAnalysis, dict);
					if (num2 < 4)
					{
						ptr++;
					}
					else
					{
						...
						for (;;)
						{
							...
							if (ptr + num2 == ptr4)
							{
								num4 = LL32.LZ4HC_InsertAndGetWiderMatch(ctx, ptr + num2 - 2, ptr, ptr5, num2, &ptr10, &ptr9, maxNbAttempts, patternAnalysis, false, dict, LL.HCfavor_e.favorCompressionRatio);
							}
							else
							{
								num4 = num2;
							...
							else
							{
								...
								for (;;)
								{
									...
									if (ptr9 + num4 == ptr4)
									{
										num7 = LL32.LZ4HC_InsertAndGetWiderMatch(ctx, ptr9 + num4 - 3, ptr9, ptr5, num4, &ptr12, &ptr11, maxNbAttempts, patternAnalysis, false, dict, LL.HCfavor_e.favorCompressionRatio);
									}
									else
									{
										num7 = num4;
									...
									else
									{
										...
										ptr15 = ptr6;
										if (LL32.LZ4HC_encodeSequence(&ptr, &ptr6, &ptr2, num2, ptr8, limit, ptr7) != 0)
										{
											goto IL_43B;
										}
										ptr = ptr9;
						...
						if (LL32.LZ4HC_encodeSequence(&ptr, &ptr6, &ptr2, num2, ptr8, limit, ptr7) == 0)
						{
							...
							ptr15 = ptr6;
							int num4;
							if (LL32.LZ4HC_encodeSequence(&ptr, &ptr6, &ptr2, num4, ptr10, limit, ptr7) == 0)
							{
								continue;
							}
						}
		...
		public unsafe static int LZ4HC_compress_optimal(LL.LZ4_streamHC_t* ctx, byte* source, byte* dst, int* srcSizePtr, int dstCapacity, int nbSearches, uint sufficient_len, LL.limitedOutput_directive limit, bool fullUpdate, LL.dictCtx_directive dict, LL.HCfavor_e favorDecSpeed)
		{
			...
			*srcSizePtr = 0;
			...
			while (ptr2 == ptr5)
			{
				...
				LL.LZ4HC_match_t lz4HC_match_t = LL32.LZ4HC_FindLongerMatch(ctx, ptr2, iHighLimit, 3, nbSearches, dict, favorDecSpeed);
				...
				else
				{
					...
					if (lz4HC_match_t.len <= (int)sufficient_len)
					{
						...
						while (l < num2)
						{
							...
							if (fullUpdate)
							{
								lz4HC_match_t2 = LL32.LZ4HC_FindLongerMatch(ctx, ptr8, iHighLimit, 3, nbSearches, dict, favorDecSpeed);
							}
							else
							{
								lz4HC_match_t2 = LL32.LZ4HC_FindLongerMatch(ctx, ptr8, iHighLimit, num2 - l, nbSearches, dict, favorDecSpeed);
							}
							...
							if (lz4HC_match_t2.len > (int)sufficient_len || lz4HC_match_t2.len + l >= 4096)
							{
								...
								while (m < num2)
								{
									...
									else
									{
										m += mlen3;
										ptr9 = ptr6;
										if (LL32.LZ4HC_encodeSequence(&ptr2, &ptr6, &ptr3, mlen3, ptr2 - off5, limit, ptr7) != 0)
										{
											goto IL_796;
										}
									}
					...
					ptr9 = ptr6;
					if (LL32.LZ4HC_encodeSequence(&ptr2, &ptr6, &ptr3, len3, match, limit, ptr7) == 0)
					{
						continue;
					}
					IL_796:
		...
		public unsafe static int LZ4HC_compress_generic_internal(LL.LZ4_streamHC_t* ctx, byte* src, byte* dst, int* srcSizePtr, int dstCapacity, int cLevel, LL.limitedOutput_directive limit, LL.dictCtx_directive dict)
		{
			...
			LL.cParams_t cParams_t = LL32.clTable[cLevel];
			...
			{
				num = LL32.LZ4HC_compress_hashChain(ctx, src, dst, srcSizePtr, dstCapacity, (int)cParams_t.nbSearches, limit, dict);
			}
			else
			{
				num = LL32.LZ4HC_compress_optimal(ctx, src, dst, srcSizePtr, dstCapacity, (int)cParams_t.nbSearches, cParams_t.targetLength, limit, cLevel == 12, dict, favorDecSpeed);
			}
			if (num <= 0)
			{
				ctx->dirty = true;
		...
		{
			return LL32.LZ4HC_compress_generic_internal(ctx, src, dst, srcSizePtr, dstCapacity, cLevel, limit, LL.dictCtx_directive.noDictCtx);
		}
		...
		public unsafe static int LZ4HC_compress_generic_dictCtx(LL.LZ4_streamHC_t* ctx, byte* src, byte* dst, int* srcSizePtr, int dstCapacity, int cLevel, LL.limitedOutput_directive limit)
		{
			...
			{
				ctx->dictCtx = null;
				return LL32.LZ4HC_compress_generic_noDictCtx(ctx, src, dst, srcSizePtr, dstCapacity, cLevel, limit);
			}
			...
			return LL32.LZ4HC_compress_generic_internal(ctx, src, dst, srcSizePtr, dstCapacity, cLevel, limit, LL.dictCtx_directive.usingDictCtxHc);
		}
		...
		public unsafe static int LZ4HC_compress_generic(LL.LZ4_streamHC_t* ctx, byte* src, byte* dst, int* srcSizePtr, int dstCapacity, int cLevel, LL.limitedOutput_directive limit)
		{
			...
			return LL32.LZ4HC_compress_generic_dictCtx(ctx, src, dst, srcSizePtr, dstCapacity, cLevel, limit);
		}
		...
		public unsafe static int LZ4_compressHC_continue_generic(LL.LZ4_streamHC_t* LZ4_streamHCPtr, byte* src, byte* dst, int* srcSizePtr, int dstCapacity, LL.limitedOutput_directive limit)
		{
			...
			return LL32.LZ4HC_compress_generic(LZ4_streamHCPtr, src, dst, srcSizePtr, dstCapacity, (int)LZ4_streamHCPtr->compressionLevel, limit);
		}
		...
		public unsafe static int LZ4_compress_HC_continue(LL.LZ4_streamHC_t* LZ4_streamHCPtr, byte* src, byte* dst, int srcSize, int dstCapacity)
		{
			...
			{
				return LL32.LZ4_compressHC_continue_generic(LZ4_streamHCPtr, src, dst, &srcSize, dstCapacity, LL.limitedOutput_directive.limitedOutput);
			}
			return LL32.LZ4_compressHC_continue_generic(LZ4_streamHCPtr, src, dst, &srcSize, dstCapacity, LL.limitedOutput_directive.notLimited);
		}
		...
		{
			return LL32.LZ4_compressHC_continue_generic(LZ4_streamHCPtr, src, dst, srcSizePtr, targetDestSize, LL.limitedOutput_directive.fillOutput);
		}
		...
		public unsafe static int LZ4_compress_HC_destSize(LL.LZ4_streamHC_t* state, byte* source, byte* dest, int* sourceSizePtr, int targetDestSize, int cLevel)
		{
			...
			return LL32.LZ4HC_compress_generic(ptr, source, dest, sourceSizePtr, targetDestSize, cLevel, LL.limitedOutput_directive.fillOutput);
		}
		...
		public unsafe static int LZ4_compress_HC_extStateHC_fastReset(LL.LZ4_streamHC_t* state, byte* src, byte* dst, int srcSize, int dstCapacity, int compressionLevel)
		{
			...
			if (dstCapacity < LL.LZ4_compressBound(srcSize))
			{
				return LL32.LZ4HC_compress_generic(state, src, dst, &srcSize, dstCapacity, compressionLevel, LL.limitedOutput_directive.limitedOutput);
			}
			return LL32.LZ4HC_compress_generic(state, src, dst, &srcSize, dstCapacity, compressionLevel, LL.limitedOutput_directive.notLimited);
		}
		...
		public unsafe static int LZ4_compress_HC_extStateHC(LL.LZ4_streamHC_t* state, byte* src, byte* dst, int srcSize, int dstCapacity, int compressionLevel)
		{
			...
			return LL32.LZ4_compress_HC_extStateHC_fastReset(state, src, dst, srcSize, dstCapacity, compressionLevel);
		}
		...
		public unsafe static int LZ4_compress_HC(byte* src, byte* dst, int srcSize, int dstCapacity, int compressionLevel)
		{
			...
			try
			{
				result = LL32.LZ4_compress_HC_extStateHC(pinnedMemory.Reference<LL.LZ4_streamHC_t>(), src, dst, srcSize, dstCapacity, compressionLevel);
			}
			finally
			{
				pinnedMemory.Free();
		...
		{
			return LL32.DeBruijnBytePos[(ulong)((val & -val) * 125613361U >> 27) * 4UL / 4UL];
		}
		...
		protected unsafe static uint LZ4_count(byte* pIn, byte* pMatch, byte* pInLimit)
		{
			...
			if (pIn < pInLimit - 3)
			{
				uint num = Mem32.PeekW((void*)pMatch) ^ Mem32.PeekW((void*)pIn);
				if (num != 0U)
				{
					return LL32.LZ4_NbCommonBytes(num);
				}
				pIn += 4;
				pMatch += 4;
			}
			...
			{
				uint num2 = Mem32.PeekW((void*)pMatch) ^ Mem32.PeekW((void*)pIn);
				if (num2 != 0U)
				{
					return (uint)((long)(pIn + LL32.LZ4_NbCommonBytes(num2) - ptr));
				}
				pIn += 4;
				pMatch += 4;
			}
			if (pIn < pInLimit - 1 && Mem.Peek2((void*)pMatch) == Mem.Peek2((void*)pIn))
			{
				pIn += 2;
				pMatch += 2;
			}
		...
		{
			LL.LZ4_putPositionOnHash(p, LL32.LZ4_hashPosition((void*)p, tableType), tableBase, tableType, srcBase);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		protected unsafe static byte* LZ4_getPosition(byte* p, void* tableBase, LL.tableType_t tableType, byte* srcBase)
		{
			return LL.LZ4_getPositionOnHash(LL32.LZ4_hashPosition((void*)p, tableType), tableBase, tableType, srcBase);
		}
		...
		public unsafe int LZ4_loadDict(LL.LZ4_stream_t* LZ4_dict, byte* dictionary, int dictSize)
		{
			...
			while (ptr == ptr2 - 4)
			{
				LL32.LZ4_putPosition(ptr, (void*)(&LZ4_dict->hashTable.FixedElementField), LL.tableType_t.byU32, srcBase);
				ptr += 3;
			}
		...
		private unsafe static readonly uint* DeBruijnBytePos = Mem.CloneArray<uint>(LL32._DeBruijnBytePos);
	}
}
