...
namespace DigitalOpus.MB.Core
{
	public class MB3_CopyBoneWeights
	{
		public static void CopyBoneWeightsFromSeamMeshToOtherMeshes(float radius, Mesh seamMesh, Mesh[] targetMeshes, Transform[][] newBonesForSMRs, Transform[] seamMeshBones, Transform[][] targMeshBones)
		{
			...
			if (seamMesh == null)
			{
				Debug.LogError(string.Format("The SeamMesh cannot be null", Array.Empty<object>()));
				return;
			}
			...
			for (int l = 0; l < targetMeshes.Length; l++)
			{
				...
				if (!(mesh == seamMesh))
				{
					...
					for (int num3 = 0; num3 < vertices2.Length; num3++)
					{
						for (int num4 = 0; num4 < list.Count; num4++)
						{
							...
							if (Vector3.Distance(vertices2[num3], vertices[num5]) <= radius)
							{
								...
								{
									Debug.LogError("Same mesh but different verts overlapped. radius too big " + num3.ToString() + "  " + num5.ToString());
								}
								num2++;
								BoneWeight boneWeight = boneWeights[num5];
								MB3_CopyBoneWeights.RemapBoneWeightIndexes(targetMeshes[l].name, ref boneWeight, array, targMeshBones[l], dictionary2, seamMeshBones);
								boneWeights2[num3] = boneWeight;
					...
					if (dictionary2.Count > 0)
					{
						...
						{
							array3[keyValuePair.Value] = seamMeshBones[keyValuePair.Key];
							array4[keyValuePair.Value] = bindposes2[keyValuePair.Key];
						}
