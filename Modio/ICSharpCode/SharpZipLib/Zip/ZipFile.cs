...
namespace ICSharpCode.SharpZipLib.Zip
{
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		private int WriteCentralDirectoryHeader(ZipEntry entry)
		{
			...
			byte[] array2 = (entry.Comment != null) ? Encoding.ASCII.GetBytes(entry.Comment) : Empty.Array<byte>();
			if (array2.Length != 0)
			{
		...
		private void CopyDescriptorBytesDirect(ZipFile.ZipUpdate update, Stream stream, ref long destinationPosition, long sourcePosition)
		{
			...
			int i = ZipFile.GetDescriptorSize(update, includingSignature);
			while (i > 0)
			{
				stream.Position = sourcePosition;
		...
		private void AddEntry(ZipFile workFile, ZipFile.ZipUpdate update)
		{
			...
			bool updateCrc = update.Entry.AESKeySize == 0;
			if (stream != null)
			{
				using (stream)
				{
					...
					{
						this.CopyBytes(update, outputStream, stream, length, updateCrc);
					}
		...
		private void CopyEntryDirect(ZipFile workFile, ZipFile.ZipUpdate update, ref long destinationPosition)
		{
			...
			destinationPosition += (long)ZipFile.GetDescriptorSize(update, includingSignature);
		}
