...
namespace Oculus.Interaction.PoseDetection.Debug
{
	public class JointRotationDebugVisual : MonoBehaviour
	{
		...
		{
			this.ResetLines();
			foreach (JointRotationActiveState.JointRotationFeatureConfig jointRotationFeatureConfig in this._jointRotation.FeatureConfigs)
			{
				Pose pose;
				JointRotationActiveState.JointRotationFeatureState jointRotationFeatureState;
				if (this._jointRotation.Hand.GetJointPose(jointRotationFeatureConfig.Feature, out pose) && this._jointRotation.FeatureStates.TryGetValue(jointRotationFeatureConfig, out jointRotationFeatureState))
				{
					this.DrawDebugLine(pose.position, jointRotationFeatureState.TargetAxis, jointRotationFeatureState.Amount);
				}
			}
		}
