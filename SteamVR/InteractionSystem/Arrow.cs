	...
	public class Arrow : MonoBehaviour
	{
		...
		private void FixedUpdate()
		{
			if (this.released && this.inFlight)
			{
				...
				this.prevVelocity = base.GetComponent<Rigidbody>().linearVelocity;
		...
		public void ArrowReleased(float inputVelocity)
		{
			...
			this.prevHeadPosition = this.arrowHeadRB.transform.position;
			this.prevVelocity = base.GetComponent<Rigidbody>().linearVelocity;
			this.SetCollisionMode(CollisionDetectionMode.ContinuousDynamic, false);
		...
		private void OnCollisionEnter(Collision collision)
		{
			if (this.inFlight)
			{
				float sqrMagnitude = base.GetComponent<Rigidbody>().linearVelocity.sqrMagnitude;
				...
				if (this.travelledFrames < 2 && !flag)
				{
					...
					Vector3 a = Vector3.Reflect(this.arrowHeadRB.linearVelocity, collision.contacts[0].normal);
					this.arrowHeadRB.linearVelocity = a * 0.25f;
					this.shaftRB.linearVelocity = a * 0.25f;
					this.travelledFrames = 0;
					return;
				}
				if (this.glintParticle != null)
				...
				if (flag2)
				{
					...
					base.transform.rotation = this.prevRotation;
					this.arrowHeadRB.linearVelocity = this.prevVelocity;
		...
		private void StickInTarget(Collision collision, bool bSkipRayCast)
		{
			...
			this.shaftRB.linearVelocity = Vector3.zero;
			...
			this.arrowHeadRB.linearVelocity = Vector3.zero;
