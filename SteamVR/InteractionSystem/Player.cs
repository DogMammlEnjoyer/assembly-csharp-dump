	...
	public class Player : MonoBehaviour
	{
		...
		public Hand leftHand
		{
			get
			{
				for (int i = 0; i < this.hands.Length; i++)
				{
					if (this.hands[i].gameObject.activeInHierarchy && this.hands[i].handType == SteamVR_Input_Sources.LeftHand)
					{
						return this.hands[i];
					}
				}
		...
		public Hand rightHand
		{
			get
			{
				for (int i = 0; i < this.hands.Length; i++)
				{
					if (this.hands[i].gameObject.activeInHierarchy && this.hands[i].handType == SteamVR_Input_Sources.RightHand)
					{
						return this.hands[i];
					}
				}
		...
		private void Awake()
		{
			...
			if (this.hmdTransforms != null)
			{
				foreach (Transform transform in this.hmdTransforms)
				{
		...
		public SteamVR_Action_Boolean headsetOnHead = SteamVR_Input.GetBooleanAction("HeadsetOnHead", false);
