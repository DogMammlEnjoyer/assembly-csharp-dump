...
namespace Technie.PhysicsCreator
{
	public class RotatedBoxFitter
	{
		public BoxDef Fit(Hull hull, Vector3[] meshVertices, int[] meshIndices)
		{
			...
			{
				hull.FindTriangles(meshVertices, meshIndices, out array, out hullIndices);
			}
		...
		public BoxDef Fit(Vector3[] hullVertices, int[] hullIndices)
		{
			...
			List<RotatedBox> list2 = RotatedBoxFitter.FindTightestBoxes(list, hullVertices);
			if (list2.Count > 0)
			{
				list2.Sort(new VolumeSorter());
				RotatedBox rotatedBox = list2[0];
				List<ConstructionPlane> list3 = new List<ConstructionPlane>();
				RotatedBoxFitter.GeneratePlaneVariants(list2[0].plane, numVariants, angleRange, list3);
				List<RotatedBox> list4 = RotatedBoxFitter.FindTightestBoxes(list3, hullVertices);
				list4.Sort(new VolumeSorter());
				RotatedBox rotatedBox2 = list4[0];
				RotatedBoxFitter.UnifyOffsets(rotatedBox2);
				return RotatedBoxFitter.ToBoxDef(rotatedBox2);
			}
		...
		private static void GeneratePlaneVariants(ConstructionPlane basePlane, int numVariants, float angleRange, List<ConstructionPlane> variantPlanes)
		{
			...
			for (int i = 0; i < numVariants; i++)
			{
				...
				float angle = Mathf.Lerp(-angleRange, angleRange, t);
		...
		{
			Vector3 b = inputBox.plane.rotation * inputBox.localCenter;
			inputBox.plane.center += b;
		...
		{
			targetHull.collisionBox = RotatedBoxFitter.ToBoxDef(computedBox);
		}
		...
		private static List<RotatedBox> FindTightestBoxes(List<ConstructionPlane> planes, Vector3[] inputVertices)
		{
			...
			{
				RotatedBox item = RotatedBoxFitter.FindTightestBox(plane, inputVertices);
				list.Add(item);
			}
			return list;
		}
