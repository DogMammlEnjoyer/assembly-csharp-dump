...
namespace Technie.PhysicsCreator
{
	public static class Utils
	{
		public static Matrix4x4 CreateSkewableTRS(Transform target)
		{
			...
			Matrix4x4 lhs = Utils.CreateSkewableTRS(target.parent);
			Matrix4x4 rhs = Matrix4x4.TRS(target.localPosition, target.localRotation, target.localScale);
			return lhs * rhs;
		}
		...
		public static Plane[] ConvertToPlanes(Mesh convexMesh, bool show)
		{
			...
			for (int i = 0; i < triangles.Length; i += 3)
			{
				...
				Vector3 normalized2 = (a2 - vector).normalized;
				...
				if (normalized3.magnitude > 0.01f)
				{
					...
					if (!Utils.Contains(plane, list))
					{
						list.Add(plane);
						if (show)
						{
							GameObject gameObject = GameObject.CreatePrimitive(PrimitiveType.Quad);
							gameObject.name = string.Format("{0} : {1} / {2} / {3}", new object[]
							{
								i,
								triangles[i],
								triangles[i + 1],
		...
		public static Mesh Clip(Mesh boundingMesh, Mesh inputMesh)
		{
			...
			foreach (Plane worldCutPlane in Utils.ConvertToPlanes(boundingMesh, false))
			{
				meshCutter.Cut(cuttableMesh, worldCutPlane);
				Mesh inputMesh2 = meshCutter.GetBackOutput().CreateMesh();
				cuttableMesh = new CuttableMesh(QHullUtil.FindConvexHull("", inputMesh2, false));
			}
		...
		{
			return -(maxProgression * -inputProgress / (inputProgress + rate));
		}
		...
		{
			return Utils.IsWeightAboveThreshold(weights.boneIndex0, weights.weight0, ownBoneIndex, minThreshold, maxThreshold) || Utils.IsWeightAboveThreshold(weights.boneIndex1, weights.weight1, ownBoneIndex, minThreshold, maxThreshold) || Utils.IsWeightAboveThreshold(weights.boneIndex2, weights.weight2, ownBoneIndex, minThreshold, maxThreshold) || Utils.IsWeightAboveThreshold(weights.boneIndex3, weights.weight3, ownBoneIndex, minThreshold, maxThreshold);
		}
		...
		{
			int num = 0;
			int ownBoneIndex = Utils.FindBoneIndex(mesh.SkinnedRenderer, bone);
			for (int i = 0; i < mesh.NumVertices; i++)
			{
				if (Utils.IsWeightAboveThreshold(mesh.BoneWeights[i], ownBoneIndex, minThreshold, maxThreshold))
				{
					num++;
				}
			}
