...
namespace UnityEngine.Animations.Rigging
{
	internal class RigSyncSceneToStreamJobBinder<T> : AnimationJobBinder<RigSyncSceneToStreamJob, T> where T : struct, IAnimationJobData, IRigSyncSceneToStreamData
	{
		public override RigSyncSceneToStreamJob Create(Animator animator, ref T data, Component component)
		{
			...
			if (syncableProperties != null)
			{
				...
				for (int m = 0; m < syncableProperties.Length; m++)
				{
					...
					{
						ref ConstraintProperties ptr = ref constraints[n];
						result.constraintWeightSyncer.BindAt(num4, animator, ptr.component, ConstraintProperties.s_Weight);
						result.modulatedConstraintWeights[num4++] = animator.BindCustomStreamProperty(ConstraintsUtils.ConstructCustomPropertyName(ptr.component, ConstraintProperties.s_Weight), CustomStreamPropertyType.Float);
						for (int num6 = 0; num6 < ptr.properties.Length; num6++)
						{
							ref Property ptr2 = ref ptr.properties[num6];
							if (ptr2.descriptor.size == 1)
							{
								result.propertySyncer.BindAt(num5++, animator, ptr.component, ptr2.name);
							}
							else
							{
								for (int num7 = 0; num7 < ptr2.descriptor.size; num7++)
								{
									result.propertySyncer.BindAt(num5++, animator, ptr.component, ptr2.name + RigSyncSceneToStreamJobBinder<T>.s_PropertyElementNames[num7]);
								}
							}
						}
					}
		...
		internal static string[] s_PropertyElementNames = new string[]
		{
			".x",
			".y",
			".z",
