...
namespace UnityEngine.Rendering.Universal
{
	internal class ClipperOffset
	{
			...
			for (int j = 0; j < this.m_polyNodes.ChildCount; j++)
			{
				...
				if (count != 0 && (delta > 0.0 || (count >= 3 && polyNode2.m_endtype == EndTypes.etClosedPolygon)))
				{
					...
					else
					{
						...
						else if (polyNode2.m_endtype == EndTypes.etClosedLine)
						{
							int num10 = count - 1;
		...
		public void Execute(ref List<List<IntPoint>> solution, double delta, int inputSize)
		{
			...
			clipper.AddPaths(this.m_destPolys, PolyTypes.ptSubject, true);
		...
		private void OffsetPoint(int j, ref int k, JoinTypes jointype)
		{
			...
			{
				IntPoint item2 = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + this.m_normals[k].X * this.m_delta), ClipperOffset.Round((double)this.m_srcPoly[j].Y + this.m_normals[k].Y * this.m_delta));
				item2.NX = this.m_normals[k].X;
				item2.NY = this.m_normals[k].Y;
				this.m_destPoly.Add(item2);
				item2 = this.m_srcPoly[j];
				item2.NX = this.m_normals[k].X;
				item2.NY = this.m_normals[k].Y;
				item2.N = (long)j;
				item2.D = 1L;
				this.m_destPoly.Add(item2);
				item2 = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + this.m_normals[j].X * this.m_delta), ClipperOffset.Round((double)this.m_srcPoly[j].Y + this.m_normals[j].Y * this.m_delta));
				item2.NX = this.m_normals[j].X;
				item2.NY = this.m_normals[j].Y;
				item2.N = (long)j;
				item2.D = 1L;
				this.m_destPoly.Add(item2);
			}
		...
		internal void DoSquare(int j, int k)
		{
			...
			IntPoint item = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + this.m_delta * (this.m_normals[k].X - this.m_normals[k].Y * num)), ClipperOffset.Round((double)this.m_srcPoly[j].Y + this.m_delta * (this.m_normals[k].Y + this.m_normals[k].X * num)));
			item.NX = this.m_normals[k].X - this.m_normals[k].Y * num;
			item.NY = this.m_normals[k].Y + this.m_normals[k].X * num;
			this.m_destPoly.Add(item);
			item = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + this.m_delta * (this.m_normals[j].X + this.m_normals[j].Y * num)), ClipperOffset.Round((double)this.m_srcPoly[j].Y + this.m_delta * (this.m_normals[j].Y - this.m_normals[j].X * num)));
			item.NX = this.m_normals[k].X + this.m_normals[k].Y * num;
			item.NY = this.m_normals[k].Y - this.m_normals[k].X * num;
			this.m_destPoly.Add(item);
		}
		...
		internal void DoMiter(int j, int k, double r)
		{
			...
			IntPoint item = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + (this.m_normals[k].X + this.m_normals[j].X) * num), ClipperOffset.Round((double)this.m_srcPoly[j].Y + (this.m_normals[k].Y + this.m_normals[j].Y) * num));
			item.NX = (this.m_normals[k].X + this.m_normals[j].X) * num;
			item.NY = (this.m_normals[k].Y + this.m_normals[j].Y) * num;
			this.m_destPoly.Add(item);
		}
		...
		internal void DoRound(int j, int k)
		{
			...
			for (int i = 0; i < num; i++)
			{
				IntPoint item = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + num2 * this.m_delta), ClipperOffset.Round((double)this.m_srcPoly[j].Y + num3 * this.m_delta));
				item.NX = num2;
				item.NY = num3;
				item.N = (long)j;
				item.D = 1L;
			...
			IntPoint item2 = new IntPoint(ClipperOffset.Round((double)this.m_srcPoly[j].X + this.m_normals[j].X * this.m_delta), ClipperOffset.Round((double)this.m_srcPoly[j].Y + this.m_normals[j].Y * this.m_delta));
			item2.NX = this.m_normals[j].X;
			item2.NY = this.m_normals[j].Y;
			item2.N = (long)j;
			item2.D = 1L;
