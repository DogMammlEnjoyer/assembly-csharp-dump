...
namespace UnityEngine.Rendering.UnifiedRayTracing
{
	internal static class TerrainToMesh
	{
		private static AsyncTerrainToMeshRequest MakeAsyncTerrainToMeshRequest(int width, int height, Vector3 heightmapScale, float[,] heightmap, bool[,] holes)
		{
			...
			ComputeTerrainMeshJob computeTerrainMeshJob = default(ComputeTerrainMeshJob);
			computeTerrainMeshJob.heightmap = new NativeArray<float>(num, Allocator.Persistent, NativeArrayOptions.ClearMemory);
			for (int i = 0; i < num; i++)
			{
				computeTerrainMeshJob.heightmap[i] = heightmap[i / width, i % width];
			}
			computeTerrainMeshJob.holes = new NativeArray<bool>((width - 1) * (height - 1), Allocator.Persistent, NativeArrayOptions.ClearMemory);
			...
			computeTerrainMeshJob.positions = new NativeArray<float3>(num, Allocator.Persistent, NativeArrayOptions.ClearMemory);
			computeTerrainMeshJob.uvs = new NativeArray<float2>(num, Allocator.Persistent, NativeArrayOptions.ClearMemory);
			computeTerrainMeshJob.normals = new NativeArray<float3>(num, Allocator.Persistent, NativeArrayOptions.ClearMemory);
			computeTerrainMeshJob.indices = new NativeArray<int>((width - 1) * (height - 1) * 6, Allocator.Persistent, NativeArrayOptions.ClearMemory);
			JobHandle jobHandle = computeTerrainMeshJob.Schedule(num, math.max(width, 128), default(JobHandle));
		...
		public static AsyncTerrainToMeshRequest ConvertAsync(Terrain terrain)
		{
			...
			return TerrainToMesh.MakeAsyncTerrainToMeshRequest(width, height, terrainData.heightmapScale, heights, holes);
		}
		...
		public static AsyncTerrainToMeshRequest ConvertAsync(int heightmapWidth, int heightmapHeight, short[] heightmapData, Vector3 heightmapScale, int holeWidth, int holeHeight, byte[] holedata)
		{
			...
			return TerrainToMesh.MakeAsyncTerrainToMeshRequest(heightmapWidth, heightmapHeight, heightmapScale, array, array2);
		}

		public static Mesh Convert(Terrain terrain)
		{
			AsyncTerrainToMeshRequest asyncTerrainToMeshRequest = TerrainToMesh.ConvertAsync(terrain);
		...
		{
			AsyncTerrainToMeshRequest asyncTerrainToMeshRequest = TerrainToMesh.ConvertAsync(heightmapWidth, heightmapHeight, heightmapData, heightmapScale, holeWidth, holeHeight, holedata);
			asyncTerrainToMeshRequest.WaitForCompletion();
			return asyncTerrainToMeshRequest.GetMesh();
		}
	}
