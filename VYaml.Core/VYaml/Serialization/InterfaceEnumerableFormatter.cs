...
{
	public class InterfaceEnumerableFormatter<[Nullable(2)] T> : IYamlFormatter<IEnumerable<T>>, IYamlFormatter
	{
		[NullableContext(1)]
		public void Serialize(ref Utf8YamlEmitter emitter, [Nullable(new byte[]
		{
			2,
			1
		})] IEnumerable<T> value, YamlSerializationContext context)
		{
			if (value == null)
			{
				emitter.WriteNull();
