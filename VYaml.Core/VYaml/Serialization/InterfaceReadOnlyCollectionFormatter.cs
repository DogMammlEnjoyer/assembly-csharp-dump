...
{
	public class InterfaceReadOnlyCollectionFormatter<[Nullable(2)] T> : IYamlFormatter<IReadOnlyCollection<T>>, IYamlFormatter
	{
		[NullableContext(1)]
		public void Serialize(ref Utf8YamlEmitter emitter, [Nullable(new byte[]
		{
			2,
			1
		})] IReadOnlyCollection<T> value, YamlSerializationContext context)
		{
			if (value == null)
			{
				emitter.WriteNull();
