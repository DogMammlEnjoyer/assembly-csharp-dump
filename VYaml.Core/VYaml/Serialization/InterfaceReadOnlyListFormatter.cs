...
{
	public class InterfaceReadOnlyListFormatter<[Nullable(2)] T> : IYamlFormatter<IReadOnlyList<T>>, IYamlFormatter
	{
		[NullableContext(1)]
		public void Serialize(ref Utf8YamlEmitter emitter, [Nullable(new byte[]
		{
			2,
			1
		})] IReadOnlyList<T> value, YamlSerializationContext context)
		{
			if (value == null)
			{
				emitter.WriteNull();
