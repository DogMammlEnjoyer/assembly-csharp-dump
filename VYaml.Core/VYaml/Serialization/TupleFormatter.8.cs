...
namespace VYaml.Serialization
{
	[NullableContext(2)]
	[Nullable(0)]
	public class TupleFormatter<T1, T2, T3, T4, T5, T6, T7, [Nullable(1)] T8> : IYamlFormatter<Tuple<T1, T2, T3, T4, T5, T6, T7, T8>>, IYamlFormatter
	{
		[NullableContext(1)]
		public void Serialize(ref Utf8YamlEmitter emitter, [Nullable(new byte[]
		{
			...
			1,
			1
		})] Tuple<T1, T2, T3, T4, T5, T6, T7, T8> value, YamlSerializationContext context)
		{
			if (value == null)
			{
				emitter.WriteNull();
			...
			context.Serialize<T8>(ref emitter, value.Rest);
			emitter.EndSequence();
		}
