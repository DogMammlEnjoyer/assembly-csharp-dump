...
namespace VYaml.Serialization
{
	[NullableContext(2)]
	[Nullable(0)]
	public class ValueTupleFormatter<T1, T2, T3, T4, T5, T6, T7, [Nullable(0)] TRest> : IYamlFormatter<ValueTuple<T1, T2, T3, T4, T5, T6, T7, TRest>>, IYamlFormatter where TRest : struct
	{
		[NullableContext(1)]
		public void Serialize(ref Utf8YamlEmitter emitter, [Nullable(new byte[]
		{
			...
			1,
			0
		})] ValueTuple<T1, T2, T3, T4, T5, T6, T7, TRest> value, YamlSerializationContext context)
		{
			...
			context.Serialize<TRest>(ref emitter, value.Rest);
			emitter.EndSequence();
		}
